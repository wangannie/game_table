            ...
            for (const frame of page.children) {
                ...
                for (const child of frame.children) {
                    if (back === null) {
                        ...
                        const item = createCard(back, child, game, frame.name, xOffset, yOffset);
...
function createCard(back, child, game, name, xOffset, yOffset) {
...
function setupMahjong(board) {
    debugger;
    console.log("here i amsetting up mahjong! lfg", board);
    const MARGIN = 300;
    const TILES_PER_ROW = 28;
    const tiles = [];
    for (const page of figma.root.children) {
            ...
            for (const frame of page.children) {
                ...
                for (const child of frame.children) {
                    if (back === null) {
                        ...
                        const item = createCard(back, child, board, // used to be game
                        frame.name, xOffset, yOffset);
                        board.appendChild(item); // used to be game
                        // item.setPluginData("gameId", game.id);
                        // item.setPluginData("class", `item-${itemIdx}`)
                        tiles.push(item);
                        item.rotation = rotation;
...
if (selection.length === 1 && (selection[0].name === "Catan Game Board" || figma.command === "reset_catan") && selection[0].type === "FRAME") {
    ...
    console.log("found mahjong, gonna set it up");
    debugger;
